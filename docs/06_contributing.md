...
この拡張機能は MIT ライセンスで公開しています。フォーク・改変・再配布は自由です。自社の用語に合わせてカスタマイズしたり、改善した版を公開・共有したりすることを歓迎します。

もし翻訳の修正や改善を行って、その内容を共有してくれる場合は以下の方法でお願いします。定常的な運用・保守を行っているプロジェクトではないため、個別の対応を保証するものではありませんが、共有いただいた内容は今後の参考にします。
